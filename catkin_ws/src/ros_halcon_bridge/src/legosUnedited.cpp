...
namespace HalconCpp{
    ...
    void callback(const sensor_msgs::Image& source){
      ...
      // ReadImage(&ho_Image, "/home/mickboe1/Halcon_exports/pieces.jpg");
      /*
     ...
     HTuple  hv_Rectangularity, hv_Ra, hv_Rb, hv_Phi;
    ...
    //  Threshold(ho_ImageChannel3, &ho_IntersectionYellowRed, 13, 42);
     //
      ...
      try
      {
        ...
        // SelectShape(ho_ConnectedRegions, &ho_SelectedReq, "rectangularity", "or", 0.95,1);
        // ho_ConnectedRegions = ho_SelectedReq.SelectShape("rectangularity", "or", 0.95,1);
        // SelectShape(ho_SelectedReq, &ho_SelectedReqSized, "area", "and", 5, 99999);
       ...
       //  if (HDevWindowStack::IsOpen())
        ...
        ClearWindow(w);
       //  if (HDevWindowStack::IsOpen())
      //  ((HRegion)ho_RegionDilation2).DispRegion(w);
       ((HImage)ho_Image).DispImage(w);
       //  DispObj(ho_Image, w);
       //  if (HDevWindowStack::IsOpen())
       SetColor(w,"cyan");
       //  if (HDevWindowStack::IsOpen())
       //    DispObj(ho_SelectedReqSized, HDevWindowStack::GetActive());
        //
       //  if (HDevWindowStack::IsOpen())
       ...
       //  float r = hv_Row.I();
       //  std::cout << (float)hv_Row[0] << '\n';
       //  std::cout << r << '\n';
       // DispLine(w, hv_Row, hv_Column, hv_Row+hv_arrow_length*hv_Phi.TupleCos(),hv_Column+hv_arrow_length*hv_Phi.TupleSin());
       //  std::cout << (float)hv_Sin[0] << '\n';
       //  std::cout << (float)hv_Row[0] << '\n';
       //  std::cout << (float)hv_Column[0] << '\n';
       //  DispArrow(w, hv_Row, 10, 100, 100, 1);
      //  float pxbycm = (( 640.0 / 43.0) + (480 / 33.0)) / 2;
        float pxbycm = 17.0;
